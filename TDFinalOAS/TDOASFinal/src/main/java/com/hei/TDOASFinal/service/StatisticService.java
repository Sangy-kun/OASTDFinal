...
public class StatisticService {
    ...
    public List<CollectivityOverallStatistics> getAllGlobalStatistics(LocalDate from, LocalDate to) {
        ...
        try {
            ...
            for (String id : ids) result.add(buildOverallStat(id, from, to));
            return result;
        } catch (SQLException e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage());
        }
    ...
    private CollectivityOverallStatistics buildOverallStat(String collectivityId,
                                                           LocalDate from,
        ...
        for (Member m : members) {
            ...
            totalRate += attendanceRepository.computeAttendanceRate(
                    m.getId(), collectivityId, from, to, activityRepository);
        }

        double percentage = totalMembers > 0 ? (double) upToDateCount / totalMembers * 100.0 : 0.0;
        double globalRate = totalMembers > 0 ? totalRate / totalMembers : 0.0;
        ...
        stat.setCollectivityInformation(getCollectivityInformation(collectivityId));
    ...
    private double calculateExpectedAmount(MembershipFee fee, LocalDate from, LocalDate to) {
        ...
        while (!current.isAfter(to)) {
            ...
            else if (fee.getFrequency() == Frequency.WEEKLY)    current = current.plusWeeks(1);
